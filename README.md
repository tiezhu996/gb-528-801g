...
4. 联锁评估检查设备载荷、线速度、行程端点、同安全区重叠、同设备跨 Cue 时序和依赖完成间隔。同一设备被两条 Cue 同时驱动产出 invalid 证据；首尾相接的接力点要求前一条终点位置与后一条起点位置一致，位置差产出 blocker 证据。每条证据包含规则编号、Cue、设备、时间窗口、实际值、阈值、单位、严重度和说明。

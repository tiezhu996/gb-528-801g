...
- `RehearsalRun`：不可覆盖的 Cue/规则版本快照、动作时间线、规则结果、碰撞窗口、同设备跨 Cue 争用证据、最高严重度与人工复核记录。
...
5. 同设备跨 Cue 争用是不依赖规则启停的内建检查：把同一设备的动作按时间排开，不同 Cue 的动作时间窗若按半开区间重叠，产出 `invalid` 证据（`DEVICE-CONTENTION`，实际值为重叠毫秒数）；首尾相接（前一条终点恰为后一条起点）是合法接力，但前一条 `to_position` 必须与后一条 `from_position` 一致，否则产出 `blocker` 证据（`DEVICE-HANDOFF`，实际值为位置差米数）。两窗之间存在空闲间隔时设备可重新布位，不做连续性要求。
6. `cue_set_version` 由 Cue ID/版本和启用规则 ID/版本稳定散列得到；相同锁定版本与规则版本生成相同时间线和规则证据。
